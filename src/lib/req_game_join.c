...
static int joinLocalParty(NimbleServerLocalParties* parties, NimbleServerParticipants* gameParticipants,
                          NimbleServerTransportConnection* transportConnection,
                          const NimbleSerializeJoinGameRequest* joinRequest, StepId latestAuthoritativeStepId,
                          struct NimbleServerLocalParty** outConnection)
{
    ...
    switch (joinRequest->joinGameType) {
        ...
        case NimbleSerializeJoinGameTypePartySecret: {
            NimbleServerLocalParty* foundPartyFromSecret = nimbleServerLocalPartiesFindParty(
                parties, joinRequest->partyAndSessionSecret.partyId);
            if (foundPartyFromSecret != 0) {
                if (foundPartyFromSecret->participantReferences.participantReferenceCount != joinRequest->playerCount) {
                ...
                CLOG_C_NOTICE(&parties->log, "could not find connection with secret. Probably timed out. trying to add "
    ...
    int errorCode = nimbleServerLocalPartiesCreate(parties, gameParticipants, transportConnection, joinRequest->players,
                                                   latestAuthoritativeStepId, joinRequest->playerCount, outConnection);
    if (errorCode < 0) {
        *outConnection = 0;
        return errorCode;
    }
...
static int nimbleServerReadAndJoinParticipants(NimbleServerLocalParties* parties,
                                               ...
                                               StepId latestAuthoritativeStepId, struct NimbleServerLocalParty** party)
{
    int errorCode = joinLocalParty(parties, gameParticipants, transportConnection, request, latestAuthoritativeStepId,
                                   party);
    if (errorCode < 0) {
                            ...
                            FldInStream* inStream, FldOutStream* outStream)
{
    ...
    int errorCode = nimbleServerReadAndJoinParticipants(&self->localParties, &self->game.participants,
                                                        transportConnection, &request,
                                                        self->game.authoritativeSteps.expectedWriteId, &party);
    if (errorCode < 0) {
        CLOG_WARN("couldn't find game session")
        nimbleSerializeServerOutJoinGameOutOfParticipantSlotsResponse(outStream, request.requestId, &self->log);
        return errorCode;
    }
    ...
    gameResponse.requestId = request.requestId;
    ...
    for (size_t i = 0; i < party->participantReferences.participantReferenceCount; ++i) {
        const NimbleServerParticipant* sourceParticipant = party->participantReferences.participantReferences[i];
    ...
    CLOG_C_DEBUG(&self->log, "join game response party %u stateID: %04X participant count: %zu secret: %" PRIX64,
