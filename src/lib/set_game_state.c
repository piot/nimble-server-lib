...
static int discardOldStepsBehindLatestState(NbdGame* game, StepId latestStateStepId)
{
    ...
    if (countDiscarded < 0) {
        CLOG_SOFT_ERROR("we couldn't discard up to %08X", oldestStepIdToKeep);
        return countDiscarded;
    }

    if (countDiscarded > 0) {
        // CLOG_DEBUG("authoritative: discarded steps earlier than %08lX (discarded %d)", oldestStepIdToKeep,
        // countDiscarded);
    }
    return countDiscarded;
}
...
int nbdGameSetGameState(NbdGame* game, StepId stepId, const uint8_t* gameState, size_t gameStateOctetCount)
{
    ...
    if (state->octetCount != 0 && stepId <= state->stepId) {
        CLOG_DEBUG("ignoring old game state. we have %08X, but tried to set %08X", state->stepId, stepId);
        return 0;
    }

    if (state->capacity < gameStateOctetCount) {
        CLOG_SOFT_ERROR("can not set gamestate. Not enough capacity %zu vs %zu", gameStateOctetCount, state->capacity);
        return -4;
    }
    ...
    CLOG_DEBUG("Server now has a new state for stepId:%08X and octetCount:%zu", stepId, gameStateOctetCount);
