...
int nimbleServerParticipantsPrepare(NimbleServerParticipants* self, uint32_t participantId,
                                    struct NimbleServerParticipant** outConnection)
{
    ...
    for (size_t i = 0; i < self->participantCapacity; ++i) {
        ...
        if (!participant->isUsed) {
            ...
            CLOG_C_DEBUG(&self->log, "allocating participant at index: %zu with unique id: %zu", i, participant->id)
            ...
            *outConnection = participant;
            ...
            //CLOG_C_DEBUG(&self->log, "participant at %zu is already used.", i)
        }
    }
...
int nimbleServerParticipantsJoin(NimbleServerParticipants* self, const NimbleServerParticipantJoinInfo* joinInfo,
                                 size_t localParticipantCount, NimbleServerParticipant** results)
{
    ...
    CLOG_C_DEBUG(&self->log, "the %zu participant(s) can join, the game participant pool is at %zu/%zu",
    ...
    for (size_t i = 0; i < self->participantCapacity; ++i) {
        ...
        if (!participant->isUsed) {
            ...
            CLOG_C_DEBUG(&self->log, "allocating participant %zu at index: %zu with unique id: %zu", joinIndex, i,
                         participant->id)
            ...
            if (joinIndex == localParticipantCount) {
                CLOG_C_DEBUG(&self->log, "all are allocated, so breaking out of loop. i:%zu, joinIndex:%zu, count:%zu",
                             i, joinIndex, localParticipantCount)
                break;
            }
        } else {
